...
def send_predict(url: str, port: int, image_paths: list[str], debug: bool = False, timeout: int = 180) -> dict:
    endpoint = f"{url.rstrip('/')}:{port}/{'predict_debug' if debug else 'predict'}"
        ...
        for cls in class_names:
            ...
            for i in range(n):
                for j in range(i + 1, n):
                    ...
                    resp = send_predict(ns.url, ns.port, image_paths, debug=ns.debug, timeout=ns.timeout)
                    results.append({
